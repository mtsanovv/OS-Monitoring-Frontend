sap.ui.jsview(OS_MONITORING_VIEW_MAIN, {
   
   getControllerName: function() {
      return OS_MONITORING_CONTROLLER_MAIN;
   },

   createContent: function(oController) {
      const toolPage = new sap.tnt.ToolPage(OS_MONITORING_PAGE_MAIN, { sideExpanded: false });
   ...
   createChangeThemeDialog: function(toolPage) {
      ...
      const dialog = new sap.m.Dialog('changeThemeDialog', { title: "Pick a theme", icon: "sap-icon://palette" });
      const closeButton = new sap.m.Button('changeThemeDialogCloseButton', { text: 'Close' , type: sap.m.ButtonType.Emphasized });
      ...
      for(const theme of THEMES) {
         ...
         themeItem.setType(sap.m.ListType.Active)
                  .attachPress(() => {
   ...
   createSideNavToggleButton: function(toolPageHeader) {
      const oController = this.getController();
      const sideNavToggleButton = new sap.m.Button(SIDE_NAV_TOGGLE_BUTTON, { icon: "sap-icon://menu2", type: "Transparent" });
   ...
   createTitle: function(toolPageHeader) {
      ...
      toolPageHeader.addContent(new sap.m.Title({ text: "OS Monitoring", level: "H1", textAlign: sap.ui.core.TextAlign.Center }));
      toolPageHeader.addContent(new sap.m.ToolbarSpacer());
   },

   createChangeThemeButton: function(toolPageHeader) {
      const oController = this.getController();
      const changeThemeButton = new sap.m.Button({ icon: "sap-icon://palette", type: "Transparent", tooltip: "Change Theme" });
   ...
   createToolPageSideContent: function(toolPage) {
      ...
      for(const item of NAV_CONTENT) {
         sideNavigationList.addItem(new sap.tnt.NavigationListItem(item.id, { icon: item.icon, text: item.text, href: "#/" + item.route }).setKey(item.id));
      }
   ...
   createToolPageMainContent: function(toolPage) {
      const oApp = new sap.m.App(OS_MONITORING_APP);
